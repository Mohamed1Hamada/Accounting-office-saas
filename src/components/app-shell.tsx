        ...
        <nav className="mt-7 space-y-1">
          {visibleNavItems.map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "block rounded-2xl px-4 py-3 text-sm font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-950",
              )}
            >
              {item.label}
            </Link>
      ...
      <div className="lg:pr-72">
        <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/85 px-5 py-4 backdrop-blur lg:hidden">
          <div className="flex items-center justify-between">
            ...
            <Link href="/settings" className="rounded-xl border border-slate-200 px-3 py-2 text-sm">
              الإعدادات
            </Link>
          </div>
          ...
          <nav className="mt-4 flex gap-2 overflow-x-auto pb-1">
            {visibleNavItems.slice(0, 7).map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="shrink-0 rounded-xl bg-slate-100 px-3 py-2 text-xs font-semibold"
              >
                {item.label}
              </Link>
            ))}
